...
create_report_file() {
    ...
    local owner_email=$2
    local out_file="$output_dir/${subscription_name}_orphaned_resources.txt"
    ...
    if grep -q "\[warning\]" "$out_file"; then
        ...
        echo "Orphaned resources check completed for $subscription_name. Check $out_file for details."
    fi
}
